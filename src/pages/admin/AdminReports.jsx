...
export default function AdminReports() {
  ...
  // Metadata de datasets/columnas desde el backend (fuente de verdad).
  useEffect(() => {
    reportsApi
      .meta()
      .then(setMeta)
  ...
  // Al cambiar de dataset: seleccionar todas las columnas, limpiar filtros.
  useEffect(() => {
    if (!dsMeta) return;
  ...
  // Sucursales de la empresa seleccionada (para el filtro sucursalId).
  useEffect(() => {
  ...
  // Estimado de registros con debounce.
  useEffect(() => {
    if (!dsMeta) return;
    setCounting(true);
    const t = setTimeout(() => {
  ...
  function renderFilter(name) {
    ...
    if (name === "sucursalId") {
      return (
        <select {...common} disabled={!filters.companyId}>
          <option value="">{filters.companyId ? "Todas" : "Elige una empresa primero"}</option>
          {sucursales.map((s) => (
